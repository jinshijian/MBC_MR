...
```{r}
mbc_Patoine %>% 
  arrange(Reference, Year) -> mbc_Patoine_arrange
# write.csv(mbc_Patoine_arrange, "../rawdata/mbc_Patoine_arrange.csv")
```
...
lm(mbc_metadata$MBC_ROM ~ mbc_metadata$Magnitude_W) %>% summary()
...
mbc_metacsv_ndvi %>% 
  mutate(tmean_c = tmean/10-273.15) -> mbc_metacsv_ndvi

panel_slr_v3 + 
  geom_point(aes(tmean_c, Cmic), data = mbc_metacsv_ndvi, col = "red") +
  geom_smooth(method = "lm", data = mbc_metacsv_ndvi, col = "red") +
  annotate("text", x = -2.5, y = 850, col = "red", hjust = 0,
           label = expression("y="~'197.96-13.17x'~(R[adj]^2~"=0.28, p<0.01, n=106")))

# ggsave(here("output/figures", "Figure_X4.png"), width = 8, height = 6)

summary(lm(Cmic ~ tmean_c, data = mbc_metacsv_ndvi))
...
sum_slope <- function(sdata, ID, Scenario, n){
  sdata %>% 
    dplyr::select(slope, outslope, slope_p, slope_se) %>% 
    summarize_all(.funs = mean) %>% 
    # rename(Mean = slope) %>% 
    transmute(ID = ID,
              Scenario = Scenario,
